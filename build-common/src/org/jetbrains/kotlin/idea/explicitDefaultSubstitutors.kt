...
object JvmTargetDefaultSubstitutor : ExplicitDefaultSubstitutor() {
    ...
    private val argument: Argument? by lazy { substitutedProperty.findAnnotation() }
    private fun prepareSubstitution(default: String): List<String> = argument?.let { listOf(it.value, default) }.orEmpty()

    override val oldSubstitution: List<String> get() = prepareSubstitution(oldDefault)
    override val newSubstitution: List<String> get() = prepareSubstitution(newDefault)

    override fun isSubstitutable(args: List<String>): Boolean = args.all { arg ->
        when (arg) {
            argument?.value, argument?.shortName, argument?.deprecatedName -> false
            else -> true
        }
    }
}
