...
internal class BitcodeCompiler(val context: Context) {
    ...
    private fun clang(configurables: ClangFlags, file: BitcodeFile): ObjectFile {
        ...
        val targetTriple = platform.targetArg.copy(
                os = if (configurables is AppleConfigurables) {
                    "${platform.targetArg.os}${configurables.osVersionMin}"
                } else {
                    platform.targetArg.os
                }
        )
