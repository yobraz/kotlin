...
object ParsedCodeMetaInfoRenderer : CodeMetaInfoRenderer {
    override fun asString(codeMetaInfo: CodeMetaInfo, registeredDirectives: RegisteredDirectives): String? {
        if (codeMetaInfo !is ParsedCodeMetaInfo) return null
        return codeMetaInfo.tag + CodeMetaInfoRenderer.getAttributesString(codeMetaInfo) + (codeMetaInfo.description?.let { "(\"$it\")" } ?: "")
    }
}
