    ...
    private fun addReifiedParametersFromSignature() {
        for (type in irClass.superTypes) {
            for (supertypeArgument in (type as? IrSimpleType)?.arguments ?: emptyList()) {
                if (supertypeArgument is IrTypeProjection && supertypeArgument.type.isReifiedTypeParameter) {
                    reifiedTypeParametersUsages.addUsedReifiedParameter(supertypeArgument.type.classifierOrFail.cast<IrTypeParameterSymbol>().owner.name.asString())
                }
            }
        }
    }
