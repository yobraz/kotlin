...
internal class SyntheticAccessorLowering(val context: JvmBackendContext) : IrElementTransformerVoidWithContext(), FileLoweringPass {
    ...
    private fun IrSimpleFunction.accessorName(superQualifier: IrClassSymbol?): Name {
        ...
        val suffix = when {
            ...
            (currentClass?.irElement as? IrClass)?.origin == JvmLoweredDeclarationOrigin.DEFAULT_IMPLS &&
                    (currentClass?.irElement as? IrClass)?.parentAsClass == parentAsClass ->
                if (!DescriptorVisibilities.isPrivate(visibility)) "\$jd" else ""
