...
class IrElementToJsStatementTransformer : BaseIrElementToJsNodeTransformer<JsStatement, JsGenerationContext> {
    ...
    override fun visitBlock(expression: IrBlock, context: JsGenerationContext): JsStatement {
        val newContext = if (expression is IrReturnableBlock) {
            expression.inlineFunctionSymbol?.let {
        ...
        val block = JsBlock(expression.statements.map { it.accept(this, newContext ?: context) })

        if (expression is IrReturnableBlock) {
            val label = context.getNameForLoop(expression)
    ...
    override fun visitReturn(expression: IrReturn, context: JsGenerationContext): JsStatement {
        ...
        val produceFinalJsNode: (JsExpression) -> JsStatement =
            if (targetSymbol is IrReturnableBlockSymbol) {
                // TODO missed value
                { JsBreak(context.getNameForLoop(targetSymbol.owner)!!.makeRef()) }
            } else {
                { JsReturn(it) }
            }

        return expression.value.maybeOptimizeIntoSwitch(context, produceFinalJsNode).withSource(expression, context)
    }
...
internal fun <T : JsNode> T.withSource(node: IrElement, context: JsGenerationContext): T {
    addSourceIfNeed(node, context)
    return this
}

private fun <T : JsNode> T.addSourceIfNeed(node: IrElement, context: JsGenerationContext) {
    if (!context.staticContext.sourcemaps) return
    ...
    (if (this is JsExpressionStatement) this.expression else this).source(JsLocation(path, startLine, startColumn))
}
