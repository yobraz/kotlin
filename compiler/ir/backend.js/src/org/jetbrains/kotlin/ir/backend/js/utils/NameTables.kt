...
class LocalNameGenerator(parentScope: NameScope) : IrElementVisitorVoid {
    ...
    val localLoopNames = NameTable<IrExpression>()
    ...
    override fun visitReturn(expression: IrReturn) {
        ...
        if (targetSymbol is IrReturnableBlockSymbol) {
            targetSymbol.owner
            persistLoopName(SYNTHETIC_BLOCK_LABEL, targetSymbol.owner)
        }
    ...
    private fun persistLoopName(label: String, loop: IrReturnableBlock) {
        localLoopNames.declareFreshName(loop, label)
    }
}
