...
internal class DefaultCallInterceptor(override val interpreter: IrInterpreter) : CallInterceptor {
            ...
            this.interpret(
                {
                    ...
                    this.pushSimpleInstruction(irCall)
                    valueArguments.forEach { this.pushState(it) }
                },
    ...
    override fun interceptCall(call: IrCall, irFunction: IrFunction, args: List<State>, defaultAction: () -> Unit) {
        ...
        when {
            ...
            irFunction.body == null && irFunction.hasBackingField() -> callStack.pushCompoundInstruction(irFunction.createGetField())
