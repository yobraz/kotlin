...
abstract class InlineCodegen<out T : BaseExpressionCodegen>(
    ...
    private fun inlineCall(nodeAndSmap: SMAPAndMethodNode, isInlineOnly: Boolean): InlineResult {
        ...
        if (maskStartIndex != -1) {
            val defaultLambdaIsCrossinline = computeDefaultLambdaOffsets()
            val infos = expandMaskConditionsAndUpdateVariableNodes(
                node, maskStartIndex, maskValues, methodHandleInDefaultMethodIndex, defaultLambdaIsCrossinline.keys
            )
            val parameters = invocationParamBuilder.buildParameters()
            for (info in infos) {
                val lambda = DefaultLambda(info, defaultLambdaIsCrossinline[info.offset]!!, sourceCompiler)
    ...
    abstract fun computeDefaultLambdaOffsets(): Map<Int /* offset */, Boolean /* is crossinline */>
