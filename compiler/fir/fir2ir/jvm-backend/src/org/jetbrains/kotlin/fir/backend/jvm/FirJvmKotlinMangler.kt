...
class FirJvmKotlinMangler(private val session: FirSession) : AbstractKotlinMangler<FirDeclaration>(), FirMangler {
    ...
    override fun FirDeclaration.mangleString(): String
        = getMangleComputer(MangleMode.FULL, { it }).computeMangle(this)

    override fun FirDeclaration.signatureString(): String
        = getMangleComputer(MangleMode.SIGNATURE, { it }).computeMangle(this)

    override fun FirDeclaration.fqnString(): String
        = getMangleComputer(MangleMode.FQNAME, { it }).computeMangle(this)
