...
import org.jetbrains.kotlin.kapt3.ir.IrToJavaStubsConverter
...
abstract class AbstractKotlinKapt3Test2 : KotlinKapt3TestBase() {
    companion object {
    ...
    override fun doMultiFileTest(wholeFile: File, files: List<TestFile>) {
        ...
        val txtFile = File(wholeFile.parentFile, wholeFile.nameWithoutExtension + ".ir.txt")

        val analysisResult =
            JvmResolveUtil.analyzeAndCheckForErrors(
                project, myFiles.psiFiles, myEnvironment.configuration, myEnvironment::createPackagePartProvider
            )
        analysisResult.throwIfError()

        val backendContext = KaptIrProducer(myEnvironment.configuration)
            .transformToIr(project, analysisResult.moduleDescriptor, analysisResult.bindingContext, myFiles.psiFiles)
        ...
        try {
            kaptContext = KaptIrContextForStubGeneration(
                options, true, logger, project, backendContext
            )
    ...
    protected fun convert(
        ...
        val converter = IrToJavaStubsConverter(kaptContext)
...
open class AbstractClassFileToSourceStubConverterTest2 : AbstractKotlinKapt3Test2(), CustomJdkTestLauncher {
    companion object {
        ...
        fun main(args: Array<String>) {
            ...
            val test = AbstractClassFileToSourceStubConverterTest2()
            try {
                test.setUp()
                test.doTest(args[0])
            } finally {
    ...
    override fun check(kaptContext: KaptIrContextForStubGeneration, javaFiles: List<File>, txtFile: File, wholeFile: File) {
//        val generateNonExistentClass = isFlagEnabled("NON_EXISTENT_CLASS", wholeFile)
        ...
        val convertedFiles = convert(kaptContext, javaFiles)
