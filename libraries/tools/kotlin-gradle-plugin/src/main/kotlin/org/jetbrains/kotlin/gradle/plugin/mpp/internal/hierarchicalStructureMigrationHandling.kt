...
private fun PropertiesProvider.checkHmppFeatureFlagsForConsistency(project: Project) {
    if (mppHierarchicalStructureByDefault) {
        if (hierarchicalStructureSupport) {
            if (project === project.rootProject) {
                if (enableGranularSourceSetsMetadata == true) {
                ...
                if (enableGranularSourceSetsMetadata == false) {
            ...
            if (nativeDependencyPropagation == true) {
                throw GradleException(
            ...
            if (project === project.rootProject) {
                ...
                if (enableGranularSourceSetsMetadata == false) { // !A!B
                    SingleWarningPerBuild.show(
                        project,
        ...
        if (project.findProperty(PropertyNames.KOTLIN_MPP_HIERARCHICAL_STRUCTURE_SUPPORT)?.toString()?.toBoolean() == false)
            throw GradleException("The property '${PropertyNames.KOTLIN_MPP_HIERARCHICAL_STRUCTURE_SUPPORT}=false' is not yet supported.")

        if (hierarchicalStructureSupport) {
            if (project === project.rootProject) {
                if (enableGranularSourceSetsMetadata == true) { // AB
                    SingleWarningPerBuild.show(
                        project,
                        "The property '${PropertyNames.KOTLIN_MPP_ENABLE_GRANULAR_SOURCE_SETS_METADATA}=true' is redundant with " +
                                "'${PropertyNames.KOTLIN_MPP_HIERARCHICAL_STRUCTURE_SUPPORT}=true'\n"
                    )
                }
                if (enableGranularSourceSetsMetadata == false) { // A!B
                    throw GradleException(
                        "Conflicting properties: '${PropertyNames.KOTLIN_MPP_HIERARCHICAL_STRUCTURE_SUPPORT}=true' and " +
                                "'${PropertyNames.KOTLIN_MPP_ENABLE_GRANULAR_SOURCE_SETS_METADATA}=false'."
                    )
                }
            }
            if (!mpp13XFlagsSetByPlugin && nativeDependencyPropagation == false) {
                SingleWarningPerBuild.show(
                    project,
                    "The property '${PropertyNames.KOTLIN_NATIVE_DEPENDENCY_PROPAGATION}=false' (enabling Kotlin/Native dependencies " +
                            "commonization) is redundant with " +
                            "'${PropertyNames.KOTLIN_MPP_HIERARCHICAL_STRUCTURE_SUPPORT}=true'\n"
                )
            }
            if (nativeDependencyPropagation == true) {
                throw GradleException(
                    "Conflicting properties: '${PropertyNames.KOTLIN_MPP_HIERARCHICAL_STRUCTURE_SUPPORT}=true' and " +
                            "'${PropertyNames.KOTLIN_NATIVE_DEPENDENCY_PROPAGATION}=true' " +
                            "(disabling Kotlin/Native dependencies commonization)."
                )
            }
        }
    }
