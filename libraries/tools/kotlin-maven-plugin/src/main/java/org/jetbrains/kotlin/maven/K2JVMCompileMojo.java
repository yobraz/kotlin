...
public class K2JVMCompileMojo extends KotlinCompileMojoBase<K2JVMCompilerArguments> {
    ...
    private ExitCode runIncrementalCompiler(
        ...
        if (javaVersionStr != null) {
            ...
            if (javaVersion >= 16) {
                try {
                    Class<?> setUnInterruptible = Class.forName("sun.nio.ch.FileChannelImpl");
                    MethodHandles.lookup().findVirtual(setUnInterruptible, "setUninterruptible", MethodType.methodType(void.class));
                }
