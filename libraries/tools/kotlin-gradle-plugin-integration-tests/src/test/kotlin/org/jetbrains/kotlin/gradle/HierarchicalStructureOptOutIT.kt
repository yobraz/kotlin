...
internal class HierarchicalStructureOptInMigrationArtifactContentIT : BaseGradleIT() {
    enum class Mode {
        FLIPPED_DEFAULT, FLIPPED_DISABLE, NON_FLIPPED_ENABLE, NON_FLIPPED_DEFAULT
    }
        ...
        build(
            *buildList {
                ...
                when (mode) {
                    ...
                    NON_FLIPPED_ENABLE -> add("-Pkotlin.mpp.hierarchicalStructureSupport=true")
                }
            }.toTypedArray(),
        ) {
            assertSuccessful()
            val metadataJarName = when (mode) {
                NON_FLIPPED_DEFAULT, FLIPPED_DEFAULT, FLIPPED_DISABLE -> "my-lib-bar-1.0.jar"
                NON_FLIPPED_ENABLE -> "my-lib-bar-1.0-all.jar"
            }
            val metadataJarEntries = ZipFile(
                projectDir.resolve("../repo/com/example/bar/my-lib-bar/1.0/$metadataJarName")
            ).use { zip ->
            ...
            val shouldHaveJvmAndJsMainEntries = when (mode) {
                ...
                FLIPPED_DEFAULT, NON_FLIPPED_ENABLE -> true
            }
